...
class VisualizationGenerator:
        ...
        for i, row in clustered_df.iterrows():
            ...
            if json_msg and 'coordinates' in json_msg:
                ...
                embedding_data = EmbeddingData(
                    ...
                    embedding_100d=np.zeros(100),
                    embedding_15d=np.array([coords.get('x', 0.0), coords.get('y', 0.0), coords.get('z', 0.0)])
                )
            else:
                no_coord_count += 1
                if i < 3:  # Log first few mismatches for debugging
                ...
                embedding_data = EmbeddingData(
                    embedding_3072d=np.zeros(3072),
                    embedding_100d=np.zeros(100),
                    embedding_15d=np.array([0.0, 0.0, 0.0])
                )
    ...
    def create_cluster_3d_plot(self, clustered_messages: List[ClusteredMessage], cluster_analyses: List[ClusterAnalysis]) -> go.Figure:
        ...
        for msg in clustered_messages:
            if msg.embeddings and msg.embeddings.embedding_15d is not None:
                coords = msg.embeddings.embedding_15d
                if len(coords) >= 3:
    ...
    def find_dendrogram_images(self, result_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        # Search for dendrogram files in the output directory
        dendrogram_dir = Path("output/dendrograms")
...
<body>
    ...
    <div class="section">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{result_data.get('cluster_statistics', {}).get('noise_points', 0)}</div>
    ...
    <div class="section">
        ...
        <p>Performance metrics and data flow through the discovery pipeline stages.</p>
        {stats_charts_html}
    </div>

    {self._generate_dendrogram_section(dendrograms)}
