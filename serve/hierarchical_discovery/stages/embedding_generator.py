...
from ..models import AtomicMessage, EmbeddedMessage, EmbeddingData, PipelineConfig

logger = get_logger(__name__)

class EmbeddingGenerator:
    """Generate embeddings with caching and dimensionality reduction"""
    ...
    def __init__(self, config: PipelineConfig):
        ...
        # Cache file path
        cache_file_template = self.embedding_config.get("cache_file", "cache/embeddings_{data_source}.pkl")
        self.cache_file = Path(cache_file_template.format(data_source=config.data_source))
        ...
        logger.info(f"EmbeddingGenerator initialized with cache: {self.cache_file}")

    def _create_cache_key(self, atomic_messages: List[AtomicMessage]) -> str:
        """Create a unique cache key based on message content"""
        # Use a hash of all atomic texts to identify this dataset
        import hashlib
        all_texts = [msg.atomic_text for msg in atomic_messages]
        content_hash = hashlib.md5('\n'.join(sorted(all_texts)).encode()).hexdigest()
        return f"{self.config.data_source}_{len(atomic_messages)}_{content_hash[:8]}"

    def load_cached_embeddings(self, atomic_messages: List[AtomicMessage]) -> Optional[Tuple[Dict[str, np.ndarray], Dict[str, Any]]]:
        """Load cached embeddings if they exist and match current data - DISABLED FOR IN-MEMORY ONLY"""
        logger.info("Embedding caching is DISABLED - generating fresh embeddings in memory")
        return None

    def save_embeddings_cache(self, embeddings: Dict[str, np.ndarray], atomic_messages: List[AtomicMessage],
                            reduction_models: Dict[str, Any]):
        """Save embeddings to cache - DISABLED FOR IN-MEMORY ONLY"""
        logger.info("Embedding caching is DISABLED - not saving to disk")
    ...
    async def generate_embeddings_batch(self, texts: List[str]) -> List[np.ndarray]:
        ...
        try:
            ...
            logger.info(f"GEMINI_API_KEY found, generating embeddings with {batch_size} batch size")
            ...
            logger.info(f"Embeddings generated successfully: {type(embeddings)}, shape: {embeddings.shape if hasattr(embeddings, 'shape') else len(embeddings)}")
            ...
            if isinstance(embeddings, list) and len(embeddings) > 0:
                ...
                if isinstance(first_embedding, np.ndarray):
                    if np.allclose(first_embedding, 0):
                        ...
                        logger.info(f"Real embeddings generated! First embedding non-zero sum: {np.sum(np.abs(first_embedding)):.3f}")
    ...
    def fit_dimensionality_reduction(self, embeddings_3072d: np.ndarray) -> Dict[str, Any]:
        ...
        logger.info("Fitting dimensionality reduction models...")
        models = {}
        ...
        if self.embedding_config.get("pca_dimensions", 100) > 0:
            ...
            logger.info(f"Fitting PCA model for {pca_dims} dimensions")
            ...
            logger.info(f"PCA explained variance ratio: {self.pca_model.explained_variance_ratio_.sum():.3f}")
        ...
        if self.embedding_config.get("umap_dimensions", 15) > 0:
            ...
            if dataset_size < 15:
                ...
                logger.info(f"UMAP n_neighbors scaling: dataset_size={dataset_size}, "
                           f"static={static_n_neighbors}, dynamic={dynamic_n_neighbors}")
                logger.info(f"Fitting UMAP model for {umap_dims} dimensions with n_neighbors={dynamic_n_neighbors}")
                ...
                logger.info(f"UMAP reduction completed with dynamic n_neighbors={dynamic_n_neighbors}")
    ...
    def apply_dimensionality_reduction(self, embeddings_3072d: np.ndarray, models: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        if 'pca' in models and self.embedding_config.get("pca_dimensions", 100) > 0:
            logger.info("Applying PCA reduction...")
    ...
    async def generate_all_embeddings(self, atomic_messages: List[AtomicMessage]) -> Dict[str, Dict[str, np.ndarray]]:
        """Generate all embeddings with caching and dimensionality reduction"""
        if not atomic_messages:
            return {}

        # Try to load from cache
        cached_result = self.load_cached_embeddings(atomic_messages)
        if cached_result:
            cached_embeddings, cache_metadata = cached_result
            # Check if cache has all required embeddings
            current_texts = [msg.atomic_text for msg in atomic_messages]
            if all(text in cached_embeddings for text in current_texts):
                logger.info("Using fully cached embeddings")
                return {text: cached_embeddings[text] for text in current_texts}

        # Generate new embeddings
        logger.info("Generating new embeddings...")
        texts = [msg.atomic_text for msg in atomic_messages]

        # Generate 3072D embeddings
        ...
        reduction_models = self.fit_dimensionality_reduction(embeddings_3072d)

        # Apply dimensionality reduction
        all_reduced = self.apply_dimensionality_reduction(embeddings_3072d, reduction_models)

        # Create embedding dictionary by text
        embeddings_by_text = {}
        for i, text in enumerate(texts):
            embeddings_by_text[text] = {
                '3072d': all_reduced['3072d'][i],
                'pca': all_reduced.get('pca', np.array([]))[i] if 'pca' in all_reduced else np.array([]),
                'umap': all_reduced.get('umap', np.array([]))[i] if 'umap' in all_reduced else np.array([])
            }

        # Save to cache
        self.save_embeddings_cache(embeddings_by_text, atomic_messages, reduction_models)

        return embeddings_by_text

    def create_embedded_messages(self, atomic_messages: List[AtomicMessage],
                               embeddings_by_text: Dict[str, Dict[str, np.ndarray]]) -> List[EmbeddedMessage]:
        """Create EmbeddedMessage objects with all embedding dimensions"""
        embedded_messages = []
        ...
        for atomic_message in atomic_messages:
            ...
            embeddings = embeddings_by_text[text]

            # Use UMAP for 15d visualization, fallback to first 3 PCA dimensions for small datasets
            embedding_15d = embeddings.get('umap')
            # Check if UMAP is None or empty array
            if (embedding_15d is None or len(embedding_15d) == 0) and embeddings.get('pca') is not None:
                # Use first 3 PCA dimensions for visualization when UMAP is skipped
                pca_embedding = embeddings.get('pca')
                if len(pca_embedding) >= 3:
                    embedding_15d = pca_embedding[:3]
                    logger.info(f"Using first 3 PCA dimensions for visualization (UMAP unavailable)")
                else:
                    embedding_15d = pca_embedding
                    logger.info(f"Using all {len(pca_embedding)} PCA dimensions for visualization (UMAP unavailable)")

            embedding_data = EmbeddingData(
                embedding_3072d=embeddings.get('3072d'),
                embedding_100d=embeddings.get('pca'),
                embedding_15d=embedding_15d,
            ...
            embedded_message = EmbeddedMessage(
                ...
                metadata=atomic_message.metadata,  # Pass through metadata including phone number
                created_at=datetime.now()
            )

            # Debug first few embedded messages with comprehensive metadata logging
            if len(embedded_messages) < 5:
                ...
                logger.info(f"EMBEDDING MESSAGE {len(embedded_messages)} (csv_row={atomic_message.csv_row_index}):")
                logger.info(f"  phone='{phone}', sent_at='{sent_at}'")
                logger.info(f"  text_snippet='{atomic_message.atomic_text[:50]}...'")
                logger.info(f"  atomic_metadata_keys={list(atomic_message.metadata.keys())}")
                logger.info(f"  embedded_metadata_keys={list(embedded_message.metadata.keys())}")
    ...
    def generate_embedding_report(self, embedded_messages: List[EmbeddedMessage]) -> str:
        ...
        has_100d = sum(1 for msg in embedded_messages if msg.embeddings.embedding_100d is not None)
        has_15d = sum(1 for msg in embedded_messages if msg.embeddings.embedding_15d is not None)
        ...
        report_lines = [
            ...
            f"  3072D (Original): {has_3072d:,} ({has_3072d/total_messages:.1%})",
            f"  100D (PCA): {has_100d:,} ({has_100d/total_messages:.1%})",
            f"  15D (UMAP): {has_15d:,} ({has_15d/total_messages:.1%})",
            "",
            "Campaign Distribution:"
        ]
