"""
firecrawl_utils.py — Firecrawl API helpers for source discovery and PDF collection.
...
def get_remaining_credits() -> int | None:
    ...
    Use this to bracket a run and compute exact credits consumed:
    ...
    try:
        app = _client()
        # get_credit_usage() is a v1-only method — may not exist on all SDK versions
...
def search_agenda_page(city: str, state: str) -> Optional[str]:
    ...
    # SDK returns SearchData with a .web list of SearchResultWeb objects
...
def extract_meeting_links(url: str, city: str, state: str) -> list[dict]:
    ...
    NOTE: Uses the LLM Extract API (~5-30 credits/call). Use validate_agenda_page
    for cheap candidate validation during discovery (1 credit/call).
    """
    app = _client()
    try:
        result = app.extract(
        ...
        # SDK returns ExtractResponse with .data attribute
        ...
        for m in meetings:
            if not isinstance(m, dict):
                continue
            # Accept any PDF url field name the LLM uses
...
def validate_agenda_page(url: str, city: str, state: str) -> dict:
    ...
    contains city council agenda content. Returns:
        {
            "valid": bool,           # True if looks like a real agenda page
            "most_recent_date": str | None,  # YYYY-MM-DD from page metadata
            "pdf_urls": list[str],   # any .pdf links found
        }

    Use this instead of extract_meeting_links when you only need to confirm
    a page is a real agenda source — extract_meeting_links uses the LLM Extract
    API (~5-30 credits/call) while this is a plain scrape (1 credit).
    """
    app = _client()
    try:
        ...
        # Extract date from page metadata (free — already fetched with scrape)
        ...
        # Keyword check: does this look like a city council agenda page?
        ...
        # Reject if looks like a news article or event listing, not an agenda index
        reject_signals = sum([
            "/article/" in url.lower(),
            "news" in url.lower() and "agenda" not in url.lower(),
            len(markdown) < 300,  # too thin to be a real agenda page
        ])
...
def scrape_pdf_text(pdf_url: str) -> Optional[str]:
    ...
    try:
        result = app.scrape(
            pdf_url,
            formats=["markdown"],
            parsers=["pdf"],  # enables PDF text extraction + OCR fallback
        )
...
def scrape_civicclerk_event_files(portal_url: str, event_id: str) -> list[str]:
    ...
    portal_url: e.g. https://blainewa.portal.civicclerk.com
    event_id: numeric event ID string
