...
GROUNDING RULES — these differ by field type:

FACT FIELDS (whatIsHappening, whatDecision, whoIsPresenting): strict source grounding. Every claim must trace verbatim to a phrase in SOURCE SECTIONS. Do not extrapolate, infer, or add context. If something is not in source, omit it. Use only the procedural framing the source uses (e.g. if source describes an appeal of a Zoning Commission denial, do not write "approve, deny, or approve with conditions" — use the source's framing of "uphold, overturn, or overturn with conditions").

FRAMING FIELDS (whyItMatters, recommendation, actionItem, askThis, supportingContext): context permitted with constraints. You may bring in general policy context, named legal precedent, or comparable jurisdictional practice — even when not stated in source — provided you:
  1. Use hedging language for any claim not in source: "often," "in similar cases," "broader patterns suggest," "comparable jurisdictions have." Do not state inferred context as established fact.
  2. State scope as scope. Two concrete examples:
     a. If the source says a Maine policy was triggered by a Texas lawsuit, do NOT write "this changed how municipalities across the country sell tax-foreclosed properties" — that asserts national scope as fact. Write "this echoes a broader pattern reshaping tax-foreclosure policy in multiple states" if you want to surface the broader pattern.
     b. If the source describes terminating a single development agreement for one business that never opened, do NOT write "this affects the city's regulatory framework for cannabis businesses" — that inflates a single-business termination into citywide framework impact. Write what is actually in scope: "this resolves an agreement for a business that never commenced operations."
  3. Never invent specific identifiers. Dollar amounts, named persons, dates, street addresses, parcel numbers, and statute citations must still appear verbatim in SOURCE SECTIONS — even in framing fields. If you don't have the specific number, omit it; do not estimate, round, or invent.

DOLLAR AMOUNT RULE (all fields): Dollar amounts may only appear if they appear verbatim in SOURCE SECTIONS. Do not infer, round, compute, or paraphrase amounts.

ADDRESS AND NAME RULE (all fields): Street addresses, parcel numbers (PPN, APN, etc.), and individual person names may only appear if they appear verbatim in SOURCE SECTIONS.
...
7. **whoIsPresenting** (optional, 50-75 words, 1-2 short paragraphs): Include this only if a presenter or responsible department is explicitly named in the SOURCE SECTIONS above — typically in a Staff Memo FROM: line or a staff report byline. Write only what the source says: name and title. Do not add "is responsible for presenting," "will be presenting," "will handle," or any inference language. STRICT RULE: If you find yourself writing "likely," "probably," "may be," "presumably," "responsible for," or "is expected to" — stop and omit the field instead. EXAMPLE — wrong vs. right: If source says "Stuart Williams, City Attorney, will inform the council of the requirements under Utah Code § 10-3-818," the field should read "Stuart Williams, City Attorney." Do NOT write "The City Attorney's office is responsible for ensuring compliance with Utah Code § 10-3-818." The second sentence asserts a responsibility for compliance that the source does not state — the source only says he is informing. CRITICAL: If the source contains "Presentation: No" (or similar wording indicating no live presentation), omit this field entirely even if a "Presented by:" name appears — that name is the document author, not a presenter.

8. **supportingContext** (optional, 50-70 words): Background context relevant to this item. May include named legal precedent, broader policy framework, comparable jurisdictional practice, or historical context drawn from general policy knowledge — using hedging language ("often," "in similar cases," "broader patterns suggest"). Specific dollar amounts, named persons, dates, addresses, and statute citations must still appear verbatim in SOURCE SECTIONS. State context as context, not scope as fact. Never repeat information already in the sections above.
